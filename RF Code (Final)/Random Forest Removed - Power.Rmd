...
imgs_cleaned <- get_images("Biotechnology_removed/data_power_removed/Biotechnology/data_processed/cell_images/", "Biotechnology_removed/data_power_removed/Biotechnology/data_processed/cell_boundaries.csv.gz", main_param, main_param)
```
...
acc_rff_powerr = mean(fit == y_test)
```
...
for (i in 1:n_sim) {
  ...
  for (j in 1:cvK) {
    ...
    rff <- randomForest::randomForest(x = image_features_train, y = as.factor(y_train))
    fit <- predict(rff, image_features_test)
    cv_acc_rff_powerr[j] = mean(fit == y_test)
  }
  cv_acc_rff_all_powerr <- append(cv_acc_rff_all_powerr, mean(cv_acc_rff_powerr))
}
boxplot(list(RF_Features = cv_acc_rff_all_powerr), ylab = "CV Accuracy", xlab = "Random Forest Model")
```

```{r}
saveRDS(rff, "rf_power_features_removed.rds")
```