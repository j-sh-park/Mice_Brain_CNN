---
title: "Random Forest Removed - Boundaries"
author: "George Wu"
date: "2023-05-07"
...
imgs_cleaned <- get_images("Biotechnology_removed/data_less_clusters_augumented/Biotechnology/data_processed/cell_images/", "Biotechnology_removed/data_less_clusters_augumented/Biotechnology/data_processed/cell_boundaries.csv.gz", main_param, main_param)
```
...
acc_rff_boundr = mean(fit == y_test)
```
...
for (i in 1:n_sim) {
  ...
  for (j in 1:cvK) {
    ...
    rff <- randomForest::randomForest(x = image_features_train, y = as.factor(y_train))
    fit <- predict(rff, image_features_test)
    cv_acc_rff_boundr[j] = mean(fit == y_test)
  }
  cv_acc_rff_all_boundr <- append(cv_acc_rff_all_boundr, mean(cv_acc_rff_boundr))
}
boxplot(list(RF_Features = cv_acc_rff_all_boundr), ylab = "CV Accuracy", xlab = "Random Forest Model")
```

```{r}
saveRDS(rff, "rf_boundaries_features_removed.rds")
```
