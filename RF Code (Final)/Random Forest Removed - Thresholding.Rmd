---
title: "Random Forest Removed - Thresholding"
author: "George Wu"
date: "2023-05-07"
...
imgs_cleaned <- get_images("Biotechnology_removed/data_thresholding_removed/Biotechnology/data_processed/cell_images/", "Biotechnology_removed/data_thresholding_removed/Biotechnology/data_processed/cell_boundaries.csv.gz", main_param, main_param)
```
...
acc_rff_threshr = mean(fit == y_test)
```
...
cvK = 5
cv_acc_rff_all_threshr = c()
cv_acc_rff_threshr = c()
...
for (i in 1:n_sim) {
  ...
  for (j in 1:cvK) {
    ...
    rff <- randomForest::randomForest(x = image_features_train, y = as.factor(y_train))
    fit <- predict(rff, image_features_test)
    cv_acc_rff_threshr[j] = mean(fit == y_test)
  }
  cv_acc_rff_all_threshr <- append(cv_acc_rff_all_threshr, mean(cv_acc_rff_threshr))
}
boxplot(list(RF_Features = cv_acc_rff_all_threshr), ylab = "CV Accuracy", xlab = "Random Forest Model")
```

```{r}
saveRDS(rff, "rf_thresholding_features_removed.rds")
```
