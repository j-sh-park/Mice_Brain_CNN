---
title: "Random Forest Merged - Denoise"
author: "George Wu"
date: "2023-05-07"
...
imgs_cleaned <- get_images("Biotechnology_merged/data_denoise/Biotechnology/data_processed/cell_images/", "Biotechnology_merged/data_denoise/Biotechnology/data_processed/cell_boundaries.csv.gz", main_param, main_param)
```
...
acc_rff_denoise = mean(fit == y_test)
```
...
cvK = 5
cv_acc_rff_all_denoise = c()
cv_acc_rff_denoise = c()
...
for (i in 1:n_sim) {
  ...
  for (j in 1:cvK) {
    ...
    rff <- randomForest::randomForest(x = image_features_train, y = as.factor(y_train))
    fit <- predict(rff, image_features_test)
    cv_acc_rff_denoise[j] = mean(fit == y_test)
  }
  cv_acc_rff_all_denoise <- append(cv_acc_rff_all_denoise, mean(cv_acc_rff_denoise))
}
boxplot(list(RF_Features = cv_acc_rff_all_denoise), ylab = "CV Accuracy", xlab = "Random Forest Model")
```

```{r}
saveRDS(rff, "rf_denoise_features.rds")
```
