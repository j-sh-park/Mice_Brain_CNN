...
imgs_cleaned <- get_images("Biotechnology_removed/data_opening_removed/data_opening_removed/Biotechnology/data_processed/cell_images/", "Biotechnology_removed/data_opening_removed/data_opening_removed/Biotechnology/data_processed/cell_boundaries.csv.gz", main_param, main_param)
```
...
#image_pixels_test = image_pixels[!sample, ]
#image_pixels_train = image_pixels[sample, ]
...
#rf <- randomForest::randomForest(x = image_pixels_train, y = as.factor(y_train))
#fit <- predict(rf, image_pixels_test)
#acc_rf = mean(fit == y_test)
...
acc_rff_openingr = mean(fit == y_test)
```
...
cvK = 5
cv_acc_rff_all_openingr = c()
cv_acc_rff_openingr = c()
...
for (i in 1:n_sim) {
  ...
  for (j in 1:cvK) {
    ...
    rff <- randomForest::randomForest(x = image_features_train, y = as.factor(y_train))
    fit <- predict(rff, image_features_test)
    cv_acc_rff_openingr[j] = mean(fit == y_test)
  }
  cv_acc_rff_all_openingr <- append(cv_acc_rff_all_openingr, mean(cv_acc_rff_openingr))
}
boxplot(list(RF_Features = cv_acc_rff_all_openingr), ylab = "CV Accuracy", xlab = "Random Forest Model")
```

```{r}
saveRDS(rff, "rf_opening_features_removed.rds")
```