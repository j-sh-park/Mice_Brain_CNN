---
title: "Random Forest Merged - Opening"
author: "George Wu"
date: "2023-05-07"
...
```{r}
#access features chosen, images, image with boundary cutoff, masked image
imgs = imgs_cleaned[[1]]
...
acc_rff_opening = mean(fit == y_test)
```
...
cvK = 5
cv_acc_rff_all_opening = c()
cv_acc_rff_opening = c()
...
for (i in 1:n_sim) {
  ...
  for (j in 1:cvK) {
    ...
    rff <- randomForest::randomForest(x = image_features_train, y = as.factor(y_train))
    fit <- predict(rff, image_features_test)
    cv_acc_rff_opening[j] = mean(fit == y_test)
  }
  cv_acc_rff_all_opening <- append(cv_acc_rff_all_opening, mean(cv_acc_rff_opening))
}
boxplot(list(RF_Features = cv_acc_rff_all_opening), ylab = "CV Accuracy", xlab = "Random Forest Model")
```

```{r}
saveRDS(rff, "rf_opening_features.rds")
```
