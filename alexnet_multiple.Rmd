...
get_images = function(img_clusters_folder_path, cell_boundaries_path, w=50, h=50)
{
  ...
  for (c in seq(to = length(clusters_ids)))
  {
    ...
    cluster_resized = mapply(mask_resize, cluster_imgs, cluster_inside, MoreArgs = list(w=224, h=224), SIMPLIFY = FALSE)
...
```{r}
imgs_cleaned <- get_images("Biotechnology_small/data_processed/cell_images/", "Biotechnology_small/data_processed/cell_boundaries.csv.gz", 224, 224)
```
