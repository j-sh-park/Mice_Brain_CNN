...
*Research Question:* Does preprocessing of images impact the ability to classify images using deep learning and machine learning?
...
### 1. With No Boundaries/Techniques (Raw Image)

```{r}
# preprocessing technique here
...
```{r}
model_boundary_power <- model_function()
```
...
```{r}
model_boundary_thresh <- model_function()
```
...
```{r}
model_boundary_open <- model_function()
```
...
```{r}
model_boundary_denoise <- model_function()
```
...
### 7. With Boundary + All Preprocessing Techniques
...
```{r}
model_boundary_all <- model_function()
```
