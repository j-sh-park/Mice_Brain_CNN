...
#2B Read in pre processed data
```{r}
clust_path = "C:/Users/James/Desktop/D3888/Biotechnology/data_processed/cell_images/"
clust_dir = list.files(clust_path)
imgs_cleaned = list(list(), c())
for(i in seq(length(clust_dir)))
{
  clust_files = list.files(paste0(clust_path, clust_dir[i]))
  for(j in seq(length(clust_files)))
  {
    img = readImage(paste0(clust_path, clust_dir[i], '/', clust_files[j]))
    imgs_cleaned[[1]] = append(imgs_cleaned[[1]], list(resize(img, 64, 64)))
    imgs_cleaned[[2]] = append(imgs_cleaned[[2]], clust_dir[i])
  }
}
```
...
model_function <- function(learning_rate = 0.001) {
  ...
  model <- keras_model_sequential() %>%
    ...
    layer_flatten() %>% 
    layer_dense(units = 448, activation = 'relu') %>% 
    layer_dropout(rate = 0.5) %>% 
    layer_dense(units = 112) %>% 
    layer_dropout(rate = 0.5) %>% 
...
imgs_masked_resized_64 = imgs_cleaned[[1]][1:400]
...
model_function2 <- function(learning_rate = 0.001) {
...
model2 <- model_function2()
model2

batch_size <- 100
epochs <- 100

yy = model2.matrix(~ as.factor(imgs_cleaned[[2]][1:400]) - 1)
#head(yy)
...
```{r}
y = as.factor(imgs_cleaned[[2]][1:400])
