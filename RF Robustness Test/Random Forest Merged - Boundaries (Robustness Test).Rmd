---
title: "Random Forest Merged - Boundaries (Robustness Test Included)"
author: "George Wu"
date: "2023-05-07"
...
imgs_cleaned <- get_images("Biotechnology_merged/data_opening/Biotechnology/data_processed/cell_images/", "Biotechnology_merged/data_opening/Biotechnology/data_processed/cell_boundaries.csv.gz", main_param, main_param)
```
...
rff <- randomForest::randomForest(x = image_features_train, y = y_train)
fit <- predict(rff, image_features_test)
acc_rff_bound = mean(fit == y_test)
```
...
for (i in 1:n_sim) {
  ...
  for (j in 1:cvK) {
    ...
    fit <- predict(rff, image_features_test)
    cv_acc_rff_bound[j] = mean(fit == y_test)
  }
  cv_acc_rff_all_bound <- append(cv_acc_rff_all_bound, mean(cv_acc_rff_bound))
}
boxplot(list(RF_Features = cv_acc_rff_all_bound), ylab = "CV Accuracy", xlab = "Random Forest Model")
```
Source: https://stackoverflow.com/questions/68271918/adding-gaussian-noise-to-each-element-of-rows-of-a-matrix-efficiently

```{r}
#calculate the sd for each row
sds <- apply(image_features_test, 1, sd)

#generate all noise factors at once and just add to image_features_test
image_features_test_robust <- image_features_test + rnorm(nrow(image_features_test) * ncol(image_features_test), mean = 0, sd = sds * 0.05)

fit <- predict(rff, image_features_test_robust)
acc_rff_bound_robust = mean(fit == y_test)
```
...
```{r}
saveRDS(rff, "rf_boundaries_features.rds")
```
