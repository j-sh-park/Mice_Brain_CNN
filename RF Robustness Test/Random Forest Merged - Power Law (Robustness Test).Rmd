---
title: "Random Forest Merged - Power Law (Robustness Test Included)"
author: "George Wu"
date: "2023-05-07"
...
imgs_cleaned <- get_images("Biotechnology_merged/data_power/Biotechnology/data_processed/cell_images/", "Biotechnology_merged/data_power/Biotechnology/data_processed/cell_boundaries.csv.gz", main_param, main_param)
```
...
for (i in 1:n_sim) {
  ...
  for (j in 1:cvK) {
    ...
    rff <- randomForest::randomForest(x = image_features_train, y = as.factor(y_train))
    fit <- predict(rff, image_features_test)
    cv_acc_rff_power[j] = mean(fit == y_test)
  }
  cv_acc_rff_all_power <- append(cv_acc_rff_all_power, mean(cv_acc_rff_power))
}
boxplot(list(RF_Features = cv_acc_rff_all_power), ylab = "CV Accuracy", xlab = "Random Forest Model")
```
...
acc_rff_power_robust = mean(fit == y_test)
```

```{r}
saveRDS(rff, "rf_power_features.rds")
```
